...
import {
  ...
  MdTranslate, MdCheckCircle, MdQuiz,
} from "react-icons/md";
...
export function AdminExamDetails() {
  ...
  useEffect(() => {
    if (!exam) {
      // If we don't have the exam from state, fetch it from the list
      // Since adminAPI doesn't have a direct getExam by ID, we fetch the list and find it
      (async () => {
        setIsLoading(true);
        try {
          const response = await adminAPI.exams.list(1, 1000);
          // Handle different response formats as in Exams.jsx
          let examsData = [];
          if (response?.items) {
          ...
          const foundExam = examsData.find(e => e.id === parseInt(examId) || e.id === examId);
          if (!foundExam) setNotFound(true);
  ...
  const loadTranslations = useCallback(async (showLoading = false) => {
    if (showLoading) setTranslationsLoading(true);
    try {
      const all = await adminAPI.exams.getTranslations(examId);
      setDoneTranslations(
  ...
  const handleTranslate = async () => {
    ...
    try {
      const response = await adminAPI.translations.trigger(examId, "exam", selectedLanguageId);
      ...
      let baseName = filename || exam?.title || 'Translated_Exam';
  ...
  if (notFound || !exam) return (
    <div className="py-20 text-center">
      ...
      <button onClick={() => navigate("/admin/exams")} className="px-6 py-3 bg-violet-600 text-white rounded-lg hover:bg-violet-700 font-medium">Back to Exams</button>
    </div>
  );
      ...
      <aside className="w-full md:w-80 flex-shrink-0 bg-white border border-slate-200 rounded-xl flex flex-col">
        <div className="px-6 py-5 border-b border-slate-100">
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-slate-500 hover:text-violet-600 font-medium transition-colors">
            <MdArrowBack className="w-5 h-5" /> Back to Exams
          </button>
        </div>
